...
public class prac {
    ...
    private static Node lca(Node root, int n1, int n2) {
        ...
        if(!get(root,n1,list1) || !get(root,n2,list2)){
            return null;
        }
    ...
    private static boolean get(Node root, int n, ArrayList<Node> list) {
        if (root==null) return false;
        list.add(root);
        if (root.data==n) return true;

        if (root.left!=null && get(root.left,n,list)) return  true;
        if (root.right!=null && get(root.right,n,list)){
            return true;
        }
